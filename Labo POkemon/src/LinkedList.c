...
int ll_remove(LinkedList* this,int index)
{
    ...
    if(this != NULL && index >= 0 && index < ll_len(this))
    {
    	...
    	if(NodoAux != NULL)
    	{
    		...
    		{
    			NodoAux2 = getNode(this,index-1);
    			NodoAux2->pNextNode = NodoAux->pNextNode;
    		}
    ...
    if(this != NULL && pFunc != NULL && (order == 1 || order == 0))
    {
    	...
    	for(int i = 0;i<len-1;i++)
    	{
    		for(int j = i+1;j<len;j++)
    		{
				...
				if((pFunc(pElementI,pElementJ)==1 && order == 1)||(pFunc(pElementI,pElementJ)==-1 && order == 0))
				{
...
/// @brief Crea y retorna una nueva lista (LinkedList), solo conteniendo los elementos que cumplan con las condiciones deceadas
///
/// @param this LinkedList* Puntero a la lista
/// @param pFunc (*pFunc) Puntero a la funcion criterio
